...
class Webcam(object):
    def __init__(self):
        self.webcam= None
    def Image_from_Webcam(self,data):
        self.webcam = data
...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    # also:
    # gi_copy = np.logical_and((gray_img>=lower_thresh), (gray_img<=upper_thresh))
    # return gi_copy.astype(float)
    
    thresh_gray_img = np.ndarray.copy(gray_img)
    size = thresh_gray_img.shape
    print(size)
    row_i = size[0]
    col_i = size[1]
    ...
    for row in range(row_i):
        for col in range(col_i):
            if thresh_gray_img[row,col]>=lower_thresh and thresh_gray_img[row,col]<=upper_thresh:
                thresh_gray_img[row,col]=1
            else: 
                thresh_gray_img[row,col]=0  
    
    print(thresh_gray_img)

    return thresh_gray_img
...
def edge_detect_naive(gray_img):
    ...
    # Sobel filters, and combine to computelegos.jpg the intensity gradient image (image with edges highlighted)
    ...
    G = np.sqrt(G_x.astype('int64')**2 + G_y.astype('int64')**2)
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    width = size[1]
    depth = size[2]
    img_r = img_d.reshape(height*width, depth)
...
def segment_image(img): 
    ...
    #binary = threshold_segment_naive(to_grayscale(img), 10, 120).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    #Here we can subscribe to the image view of webcam to return the segmented images
    rospy.init_node("webcam",anonymous=True)
    sub_image = Image_Read()
    # sub_image = rospy.Subscriber('usb_cam/image_raw', Image, queue_size=10, callback=self.Image_from_Webcam)
    test_img = read_image(sub_image, grayscale=True)
    test_img_color = read_image(sub_image)
